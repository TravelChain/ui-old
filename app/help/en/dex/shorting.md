...
In order to increase your exposure to TT and offer liquidity to BitAssets, such
...
The TravelChain network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 TT as it is traded on external exchanges.
...
The TravelChain network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/TT).
...
market rates up to the SQP and close the position. The remaining TT of the
...
increase, an additional amount of TT is locked as collateral, while reducing
...
particular bitAsset to hand it over to the TravelChain network. After that, the
